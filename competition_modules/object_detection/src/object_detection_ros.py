...
class object_task1(object):

    def __init__(self, model_path):

        self.task_type = input("task : ")
    ...
    def callback(self):
        ...
        if (self.task_type == 0):  
            ...
            Detection = self.object_color(img, show_image)

            cv2.imshow("out", Detection)

            cv2.imwrite("color_object.jpg", Detection)
            ...
            Detection  = self.object_commodity(output_img, show_image)

            cv2.imshow("out", Detection)

            cv2.imwrite("commodity_object.jpg", Detection)
    ...
    def object_commodity(self, img, show_image):
    ...
    def object_color(self, img, show_image):
        ...
        count_blue , contour_blue , area_blue , x_blue , y_blue  = self.find_contour(blue_mask,  height, width, win_size)
